...
export const studentApplicationStatus = pgEnum("student_application_status", [
  "registered",
  "ready for review",
  "approved",
  "rejected",
...
export const dormitories = pgTable(
  "Dormitories",
  {
...
export const rooms = pgTable(
  ...
  (table) => {
    return {
      ...
      roomsNumberDormitoryIdKey: unique("Rooms_number_dormitory_id_key").on(table.number, table.dormitoryId),
...
export const students = pgTable(
  ...
  (table) => {
    return {
      ...
      studentsGenderCheck: check(
        "Students_gender_check",
        sql`(gender)::text = ANY ((ARRAY['male'::character varying, 'female'::character varying])::text[])`
      ),
...
export const rentalAgreements = pgTable(
  ...
  (table) => {
    return {
      ...
      rentalAgreementsContractDateCheck: check(
        "RentalAgreements_contract_date_check",
        sql`contract_date <= CURRENT_DATE`
      ),
      rentalAgreementsPaymentCheck: check(
        "RentalAgreements_payment_check",
        sql`payment >= (0)::double precision`
      ),
    };
  }
);
...
export const studentApplications = pgTable(
  ...
  (table) => {
    return {
      ...
      studentApplicationsSubmissionDateCheck: check(
        "StudentApplications_submission_date_check",
        sql`submission_date <= CURRENT_DATE`
      ),
      studentApplicationsIncomeCheck: check(
        "StudentApplications_income_check",
        sql`income >= (0)::double precision`
      ),
    };
  }
);
...
export const employmentContracts = pgTable(
  ...
  (table) => {
    return {
      ...
      employmentContractsHireDateCheck: check(
      ...
      employmentContractsSalaryCheck: check(
        "EmploymentContracts_salary_check",
        sql`salary >= (0)::double precision`
      ),
    };
  }
);
